...
export function getTemporaryRegistrations(req, res) {
  const accountId = req.context.accountId;
  const rows = list("temporaryRegistrations").filter((item) => item.accountId === accountId);
  return ok(res, rows);
}

export function registerTemporaryRecruitment(req, res) {
  ...
  const createdRow = insert("temporaryRegistrations", {
